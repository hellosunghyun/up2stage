...
const PRIMARY: PrimaryNoticeExtract = {
  ...
  key_dates: ["신청기간: 2026. 8. 3. 10:00", "마감: 2026. 8. 10. 15:00", "결과 발표: 2026. 9. 30."],
...
describe("InitialGuidanceView", () => {
  it("renders overview and sections", () => {
    render(
      <InitialGuidanceView guidance={GUIDANCE} primaryNotice={PRIMARY} onQuickCheck={() => {}} />
    );
    expect(screen.getByText("문서를 모두 확인했어요")).toBeTruthy();
    expect(screen.getByText("주요 지원 조건")).toBeTruthy();
    expect(screen.getByText("가장 가까운 마감")).toBeTruthy();
    expect(screen.getByText("필수 제출서류 2개")).toBeTruthy();
  ...
  it("opens a canonical source from a Guidance card", () => {
    ...
    render(
      <InitialGuidanceView
        ...
        sourceLabels={{
          "src:doc-notice:p2:e7": "공고문.hwp · p.2"
        }}
        onQuickCheck={() => {}}
    ...
    fireEvent.click(screen.getByText("공고문.hwp · p.2"));
